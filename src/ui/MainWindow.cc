    ...
    m_terminalDialog(NULL)
{
    ...
    actions << ui.actionMultiMissionView;
    ...
    m_apmToolBar->setMultiFlightPlanViewAction(ui.actionMultiMissionView);
    m_apmToolBar->setMultiFlightDisplayAction(ui.actionMultiFlightDisplay);
    ...
    for(int i = 0; i < widgets.size(); ++i)
    {
        ...
        if (!dock)
        {
            ...
            case VIEW_MISSION:
                ...
                //added by:Xiangwei Niu
            case VIEW_MULTI_MISSION:
                //dock = createDockWidget(multiPlannerView,tool,tool->getTitle(),tool->objectName(),(VIEW_SECTIONS)view,location);
                dock = createDockWidget(plannerView,tool,tool->getTitle(),tool->objectName(),(VIEW_SECTIONS)view,location);
                break;
...
void MainWindow::buildCommonWidgets()
{
    ...
    if (!multiPlannerView)
    {
        multiPlannerView = new SubMainWindow(this);
        multiPlannerView->setObjectName("VIEW_MULTI_MISSION");
        multiPlannerView->setCentralWidget(new QGCMapTool(this));
        //multiPlannerView->setCentralWidget(new UASDetails(this));
        addToCentralStackedWidget(multiPlannerView, VIEW_MULTI_MISSION, "Maps");
    }
    ...
    //added by: Xiangwei Niu
    //createDockWidget(multiPlannerView,new UASListWidget(this),tr("Unmanned Systems"),"UNMANNED_SYSTEM_LIST_DOCKWIDGET",VIEW_MISSION,Qt::LeftDockWidgetArea);
    createDockWidget(multiPlannerView,new UASMultiView(this),tr("Multi Systems"),"MULTI_SYSTEM_LIST_DOCKWIDGET",VIEW_MULTI_MISSION,Qt::LeftDockWidgetArea);
    //createDockWidget(multiPlannerView,new QGCWaypointListMulti(this),tr("Mission Plan"),"WAYPOINT_LIST_DOCKWIDGET",VIEW_MULTI_MISSION,Qt::BottomDockWidgetArea);
...
void MainWindow::loadDockWidget(QString name)
{
    ...
    //added by: Xiangwei Niu
    else if (name == "MULTI_SYSTEM_LIST_DOCKWIDGET"){
        createDockWidget(centerStack->currentWidget(),new UASMultiView(this),tr("Multi Systems"),"MULTI_SYSTEM_LIST_DOCKWIDGET",currentView,Qt::RightDockWidgetArea);
    }
...
void MainWindow::loadCustomWidget(const QString& fileName, int view)
{
    ...
    if (tool->loadSettings(fileName, true))
    {
        ...
        case VIEW_MISSION:
            ...
            //added by: Xiangwei Niu
        case VIEW_MULTI_MISSION:
            createDockWidget(multiPlannerView,tool,tool->getTitle(),tool->objectName()+"DOCK",(VIEW_SECTIONS)view,Qt::LeftDockWidgetArea);
            break;
...
void MainWindow::connectCommonActions()
{
    ...
    //added by:Xiangwei Niu
    perspectives->addAction(ui.actionMultiMissionView);
    ...
    //added by: Xiangwei Niu
    if (currentView == VIEW_MULTI_MISSION)
    {
        ui.actionMultiMissionView->setChecked(true);
        ui.actionMultiMissionView->activate(QAction::Trigger);
    }
    ...
    //added by:Xiangwei Niu
    connect(ui.actionMultiMissionView, SIGNAL(triggered()), this, SLOT(loadMultiOperatorView()));
...
void MainWindow::UASCreated(UASInterface* uas)
{
    ...
    ui.actionMultiMissionView->setEnabled(true);
...
//    foreach (QString a, keys) {
//        QLOG_DEBUG() << a << " "<< settings.value(a,-1).toInt();
//    }
    ...
    else
    {
        ...
        //added by: Xiangwei Niu
        case VIEW_MULTI_MISSION:
            centerStack->setCurrentWidget(multiPlannerView);
            break;
...
void MainWindow::loadMultiOperatorView()
{
    if (currentView != VIEW_MULTI_MISSION)
    {
        storeViewState();
        currentView = VIEW_MULTI_MISSION;
        ui.actionMultiMissionView->setChecked(true);
        loadViewState();
    }
}
